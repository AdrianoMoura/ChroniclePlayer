...
| `playlistItems.list` (50/page) | 1 per call | Gap detection during sync, and on-demand back-catalog fetch when scrolling past a channel's local archive ([[B-002]], implemented 2026-07-12 — bounded at 4 pages/call, resumable). Also the video-id-collection step of importing a YouTube playlist into a local Playlist, and of checking/applying its Sync action (D-059, Pending) — the same generic call, capped at 100 pages (YouTube's own 5000-item playlist limit) rather than resumable, since each is a one-time explicit user action, not routine sync. Check and Sync each redo their own independent page-through rather than sharing a cached diff. |
| `playlists.list` (`part=snippet`, single id) | 1 per call | Source playlist's own title/description, only at import time (D-059, Pending) |
| `videos.list` (batched, 50 ids/call) | 1 per call | Hydrate duration, live status, stats. Also hydrates videos newly discovered while importing a YouTube playlist or syncing one (D-059, Pending), via the existing D-029 `upsertExternalVideo` path. |
