...
| `playlistItems.list` (50/page) | 1 per call | Gap detection during sync, and on-demand back-catalog fetch when scrolling past a channel's local archive ([[B-002]], implemented 2026-07-12 — bounded at 4 pages/call, resumable). Also the video-id-collection step of importing a YouTube playlist into a local Playlist, and of checking/applying its Sync action (D-059) — the same generic call, capped at 100 pages (YouTube's own 5000-item playlist limit) rather than resumable, since each is a one-time explicit user action, not routine sync. Check and Sync each redo their own independent page-through rather than sharing a cached diff. |
| `playlists.list` (`part=snippet`, single id) | 1 per call | Source playlist's own title/description, only at import time (D-059) |
| `videos.list` (batched, 50 ids/call) | 1 per call | Hydrate duration, live status, stats. Also hydrates videos newly discovered while importing a YouTube playlist or syncing one (D-059), via the existing D-029 `upsertExternalVideo` path. |
...
| `commentThreads.list` / `.insert` | 1 / 50 | Read/post comments ([[B-006]], implemented 2026-07-12; D-032). **Revised 2026-07-13 ([[B-081]]):** reading empirically also requires the write scope (`youtube.force-ssl`), not just readonly as originally assumed — contradicts Google's own published docs, which describe `commentThreads.list` as readonly-safe; kept as an observed-behavior note rather than resolved discrepancy, in case it turns out to be specific to this project's OAuth consent configuration. Gated the same way as every other write-scope action (proactive `write-scope-required`, dialog before consent), not with a bespoke readonly/write split. The `snippet` part already fetched also carries the viewer's own existing `viewerRating` ('like'/'none') per comment — shown as a read-only red-heart indicator, no extra call or quota. **D-063 (2026-09-01):** the panel exposes a "Top comments"/"Newest first" toggle mapped straight onto `commentThreads.list`'s own `order` param (`relevance`/`time`) — same call, same 1 unit/page, no quota impact either way. **D-070:** a comment/reply's author name now links to that user's own channel in-app, off the same `authorChannelId` field D-064 already surfaces — no new call, no extra quota. |
| `comments.insert` | 50 per call | Reply to a comment ([[B-006]], implemented 2026-07-12; D-032) |
| `comments.update` | 50 per call | Edit the viewer's own comment or reply (D-064). Same write scope as every other comment action; YouTube itself 403s if the authenticated user isn't the comment's author — Chronicle only decides when to *show* the Edit action, via the `authorChannelId` field already riding free on the same `snippet` part `commentThreads.list` fetches, compared against `channels.list mine=true`'s own `id` (also previously discarded, now surfaced through `getConnectedChannel`). |
