# Bug & Adjustment Tracker — v0.1.0 (shipped 2026-07-11)
...
Everything below is Resolved and carries no per-entry **Target** field, since 0.1.0 was
the only release that existed at the time — this section heading is the version marker.
Covers all work from the project's start through 2026-07-14: the first two dogfooding
batches (B-001–B-017 and B-054–B-066).
...
A second fix attempt landed this session (dev renderer URL now travels through relaunch
`args`, not just env-inheritance) but stayed open pending a live re-test; not moved to
Resolved. Still open as of this writing — see `../tracker-current.md` for the full history
(a third attempt has since landed, also unconfirmed) and current status.
